...
Public Class Particle
    ...
    Public Function ActiveColor() As Integer
        Try
            ...
            Return (A * 256 ^ 2 * 256) + (Color.R * 256 ^ 2) + (Color.G * 256) + Color.B
        Catch
...
Public Class ParticleSystem
    ...
    Private Function UpdateParticle(ByRef P As Particle, ByVal Deltatime As Double) As Boolean
        ...
        P.Color.R += P.ColorDelta.R * Deltatime
        P.Color.G += P.ColorDelta.G * Deltatime
        P.Color.B += P.ColorDelta.B * Deltatime
    ...
    Private Sub AdvanceColor(ByRef P As Particle, ByVal Current As Integer)
        ...
        P.Color.R = Info.tColors(Current).R
        P.Color.G = Info.tColors(Current).G
        P.Color.B = Info.tColors(Current).B
        P.ColorDelta.R = (Info.tColors(Current + 1).R - Info.tColors(Current).R) / Time
        P.ColorDelta.G = (Info.tColors(Current + 1).G - Info.tColors(Current).G) / Time
        P.ColorDelta.B = (Info.tColors(Current + 1).B - Info.tColors(Current).B) / Time
        P.LastColor = Current
    End Sub
